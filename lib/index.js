...
LoadRoutes.prototype.pathToRoute = function(target, base) {
    if (base && typeof base === 'string') {
        ...
        for (var i = target.length - 1; i >= 0; i--) {
            ...
            if (i === target.length - 1 && (segment === 'index' || segment.charAt(0) === '.'))
                continue;
            if (segment !== '')
    ...
    return '/' + ((target !== 'index' && target.charAt(0) === '.') ? target : '');
};
